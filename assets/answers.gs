...
function saveSettlement(body) {
  ...
  rows.push(['올린 시각', at, '', '', '', '']);
  rows.push([]);
  rows.push(['조', '항목', '받은 잔돈', '제출 교환권', '교환권 매출', '현금 매출',
    '행사전 구매', '매출 합계', '재료원가', '재정 지급']);
  (s.rows || []).forEach(function (r) {
    rows.push([r.name, itemLabel(r), r.float, r.final, r.voucher, r.cash, r.presale, r.total,
      r.cost, r.cost > 0 ? (r.costPaid ? '재정 지급' : '돌려드릴 것') : '']);
  });
...
/** 「4조 — 닭꼬치 3,000원 · 소떡소떡 2,000원」 처럼 한 칸에 담는다. */
function itemLabel(r) {
  return (r.items || []).map(function (x) {
    return x.name + (x.price ? ' ' + Number(x.price).toLocaleString('en-US') + '원' : '');
  }).join(' · ');
}
