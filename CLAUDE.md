...
- `voucher-settlement/` — 교환권 정산 대사(검증) 정적 웹앱. 빌드 도구 없이 파일을 그대로 서빙.
  ...
  - **`table-layout: fixed`로 칸 너비를 고정하지 말 것.** 금액이 칸보다 길어지면 「9,910,000원」이 두 줄로 접혀 표가 세로로 부풀고 장수가 늘어난다. 대신 금액 칸에 `white-space: nowrap`을 걸고, 조별 표는 「원」을 제목의 「(단위: 원)」으로 빼 폭을 줄였다.
