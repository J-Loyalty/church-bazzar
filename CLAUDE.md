...
- `forms.html` — 부스에 세울 **메뉴판**(A4 가로)과 교환권 봉투에 붙일 **마감 제출용 수량 양식**(A4 세로)을 만드는 입력 화면. 조 목록을 한 번 적으면 두 인쇄물이 함께 생성되며, 마지막 칸을 채우면 다음 칸이 자동으로 늘어난다. 입력은 `localStorage` 키 `cbz_forms_v1`에 남는다. 두 인쇄물은 용지 방향이 달라, 인쇄 시 `@page` 규칙을 바꾸고 다른 쪽 구역을 `display:none`으로 감춘다 — 한 인쇄 작업에 가로·세로를 섞지 않기 위함이다. **인쇄 CSS에 `vh`를 쓰지 말 것** — 종이가 아니라 화면 높이로 잡혀 빈 장이 끼어든다.
