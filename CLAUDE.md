...
- 상태는 `app.js` 최상단의 단일 `state` 객체(`booths`, `floats`, `finals`, `cash`, `presale`, `desk`)로 관리하며, `localStorage` 키 `cbz_voucher_v1`에 저장된다. 권종은 `DENOMS = [1000, 5000, 10000]`으로 고정.
...
- 계산은 `compute()` 한 곳에 모아 두고 화면·CSV·시트 전송이 모두 그 결과를 쓴다. 화면 그리기와 섞지 말 것.
  ...
  - 최종 수익금 = 받은 돈 + 조별 현금 매출 합계 + 행사전 구매 매출. **조가 모아 온 교환권은 더하지 않는다** — 이미 받은 돈에 들어 있어 두 번 세는 것이 된다
