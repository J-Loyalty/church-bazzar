...
- `voucher-settlement/` — 교환권 정산 대사(검증) 정적 웹앱. 빌드 도구 없이 파일을 그대로 서빙.
  ...
  - `report.html` / `report.css` / `report.js` — **결산 보고서.** `settle.js`의 결과를 A4 두 장에 옮기기만 한다. 같은 브라우저의 `localStorage`를 읽으므로 입력한 기기에서 열어야 한다. **장 나눔을 브라우저에 맡기지 말고 `.sheet` 로 직접 나눈다** — 그래야 화면 미리보기와 인쇄물이 같은 장수로 나온다. 지금 **세 장**이다 (① 1~4번: 매출·원가·수익금·대사 ② 5번: 조별 내역 ③ 6~8번과 확인란).
  - **높이는 인쇄 폭(CSS 약 703px)에서 재야 한다.** 화면 폭에서 재면 실제보다 짧게 나온다. 인쇄 가능 높이는 A4 12mm 여백 기준 약 1,032px, **Letter 용지면 약 964px**이다. 프린터가 Letter 로 맞춰져 있을 수 있으므로 **한 장을 900px 안쪽**으로 잡는다. 지금(175mm 폭 기준) 1장 775px·2장 553px·3장 459px. 인쇄 폭이 150mm 까지 좁아져도 3장을 지킨다. 보고서 본문은 14px 로, 화면 기준(17px)보다 한 단계 작다 — 종이는 화면보다 촘촘해도 읽히고, 이 한 단계가 장수를 좌우한다. **머리글·바닥글까지 켜져 있으면 900px 아래로 내려가므로** 한 장을 850px 안팎으로 잡는다.
  ...
  - **인쇄 가능한 폭을 꽉 채우지 않는다.** `@page` 여백은 10mm 지만 `.sheet`는 **175mm**로 잡아 양옆에 7.5mm 씩 남긴다 — 프린터·여백 설정에 따라 실제 폭이 조금씩 좁아지는데 꽉 채워 두면 그만큼 **오른쪽이 잘린다.** 가장 넓은 조별 표의 최소 필요 폭은 150mm 라, 그보다 좁아지지 않는 한 안전하다.
  - **`table-layout: fixed`로 칸 너비를 고정하지 말 것.** 금액이 칸보다 길어지면 「9,910,000원」이 두 줄로 접혀 표가 세로로 부풀고 장수가 늘어난다. 대신 금액 칸에 `white-space: nowrap`을 걸고, **모든 표의 「원」을 절 제목의 「(단위: 원)」으로 뺐다** — 칸마다 붙이면 표가 그만큼 넓어진다. 문장 안에서는 `money()`로 「1,000원」을 그대로 쓰고, 표 칸에는 `won()`으로 숫자만 넣는다.
  - `html, body`의 여백을 0으로 둔다 — 남아 있으면 맨 뒤에 **빈 장이 한 장 더** 나온다.
  - **쪽 번호를 종이 아래에 붙이려면 장 높이를 `mm`로 직접 잡는다.** 장을 세로 flex 로 두고 쪽 번호에 `margin-top: auto` 를 준 뒤 `.sheet { min-height: 250mm }` — `100%`는 부모 높이가 auto 라 0 으로 풀려 듣지 않고, `vh`는 종이가 아니라 화면 높이로 잡힌다. 250mm 는 A4(277mm)와 Letter(259mm) 둘 다에 들어간다.
  - **장 첫머리 여백은 `:first-of-type`이 아니라 `> :first-child` 로 없앤다.** `h2:first-of-type` 은 그 장의 첫 `h2` 를 집으므로, 앞에 다른 덩어리(`.two-col`)가 오는 장에서는 **제목이 그 덩어리에 딱 붙어 버린다.**
...
- **줄은 품목 단위이고 돈은 조 단위다.** `items`는 `{id, team, name, price}`의 배열로 **한 줄이 한 품목**이며, 4조처럼 한 조가 세 품목을 맡으면 같은 `team`이 세 줄에 되풀이된다. 나머지 맵은 모두 **조 이름을 열쇠로** 쓴다 — 교환권함이 조마다 하나라 정산 단위가 조이기 때문이다.
  ...
  - **재료원가(`costs`)와 `costPaid`는 조 단위**라 등록 표에서 **그 조가 처음 나오는 줄에만** 입력칸을 낸다. 줄마다 받으면 세 배로 잡힌다.
  - **계산 시트도 표 하나다** (A~R). 한 줄이 한 품목이고 조 단위 칸은 그 조 첫 줄에만 적는다. **줄 수를 고정하지 말 것** — 조가 15개여도 한 조가 서너 품목을 맡으면 줄은 그 몇 배가 되어 뒤쪽 조가 잘려 나간다. `calcRows(st)`가 품목 수 + 여유로 정하고 `calcMap(rows)`가 그 값을 받는다. 시트에서는 칸을 없앨 수 없으므로, 위쪽에 같은 조 이름이 이미 나왔으면 `COUNTIF`로 계산 칸을 비워 합계에 섞이지 않게 하고 맨 오른쪽 「확인」 칸이 경고를 띄운다
...
- 계산은 `compute()` 한 곳에 모아 두고 화면·CSV·시트 전송이 모두 그 결과를 쓴다. 화면 그리기와 섞지 말 것.
  ...
  - **공통 수입(`commonIncomes`)은 `voucher` 표시로 갈린다.** 켠 줄(목장쿠폰·선판매쿠폰·전도용 교환권)은 **대사 양쪽에 똑같이** 더한다 — 교환권도 대금도 아침 계수 전에 이미 오갔기 때문이다. 한쪽에만 더하면 차이가 생기고, 아예 빼면 **대사는 0으로 통과하는데 매출만 조용히 빠진다**(그래서 경고로 잡히지 않는다). 켜지 않은 줄(헌옷 수거 등)은 교환권을 거치지 않으므로 **매출에만** 더한다
  - 예전의 한 칸짜리 `presale`(조 구분 없는 행사전 구매)과 `presaleVoucher`는 이 목록으로 합쳐졌다. `normalize()`가 옛 백업의 두 값을 목록 항목으로 옮긴다
  - 미사용 교환권 = 팔려 나간 교환권 − 조별 교환권 매출 합계 (오류가 아니라 손님이 가져간 몫)
  - 매출 = 받은 돈 + 조별 현금 매출 합계 + 조별 행사전 구매 합계 + 공통 수입 중 교환권 판매가 아닌 몫. **조가 모아 온 교환권은 더하지 않는다** — 이미 받은 돈에 들어 있어 두 번 세는 것이 된다
...
- **구글 시트 연동**: [저장·백업] 탭에서 `assets/config.js`의 Apps Script 주소로 `saveSettlement`/`loadSettlement`를 보낸다. 암호는 `assets/answers.gs` 쪽에만 있다. 탭 세 개를 쓰며 **보낼 때마다 통째로 덮어쓴다**.
  ...
  - **병합한 칸을 가르며 행·열을 얼리지 말 것.** 맨 위 답 카드는 여러 칸을 병합한 것이라 `setFrozenColumns`가 그 가운데를 지나면 시트가 거부한다(「병합된 셀의 일부만 포함된 열을 고정할 수 없습니다」). 지금은 행만 얼린다 — 병합이 모두 얼린 구간 안에 들어 있어 괜찮다
  - **시트를 줄마다 건드리지 말 것.** `getRange().setValue()`는 한 건마다 왕복하므로, 70줄 × 5열이면 350번이 되어 Apps Script 가 시간 제한에 걸린다. 그러면 구글이 JSON 대신 **오류 HTML**을 돌려주고 사이트에는 「예상과 다른 응답이 왔습니다」가 뜬다 — 주소가 틀린 것처럼 보이지만 실제로는 느려서 죽은 것이다. 열 단위 `setFormulas`/`setValues`로 묶는다. `tools/check-calc-sheet.js`가 왕복 횟수에 한도(400)를 걸어 둔다
...
6. **줄바꿈(워드랩)은 어절 단위로 한다.** 브라우저는 한글을 글자 단위로 끊기 때문에, 그냥 두면 "위쪽이 안쪽(본당 방향으 / 로 추정)"처럼 낱말 중간에서 줄이 바뀌어 읽기 어렵다. 이 저장소에서는 다음을 지킨다.
   ...
   - 화면 폭: PC에서 **읽는 문서**는 최대 1120px이다(글줄이 길면 읽기 어렵다). **정산 도구는 1360px** — 읽는 글이 아니라 열이 아홉인 표가 들어가는 자리라, 960px에서는 결과 표가 안쪽에서 가로로 밀렸다. 24"(1920)·27"(2560) 화면에서 표가 통째로 보이는 폭이다. 대신 그 안의 긴 글줄은 `max-width: 90ch`로 따로 막는다. 모바일은 화면 폭에 맞춘다.
...
8. **화면도 같은 기준으로 만든다.** 문서를 읽는 사람과 화면을 쓰는 사람이 같은 사람이다.
   ...
   - **한글을 치는 중에 입력칸이 든 영역을 다시 그리지 않는다.** 한 글자가 여러 번의 `input` 이벤트로 만들어지므로(ㅈ → 조), 그 사이에 `innerHTML`로 다시 그리면 입력칸이 새 것으로 바뀌어 조합이 끊기고 **「1ㅈㅗ」처럼 깨진 채로 남는다.** 포커스와 커서를 되돌려 놓아도 소용없다 — 조합 상태는 그 DOM 노드에 붙어 있다.
     - 글자 칸은 치는 동안 **값만 상태에 담아 두고**, 표를 **아주 떠날 때**(`focusout`에서 `relatedTarget`이 표 밖일 때) 한 번 정리한다.
     - **`change`에서 다시 그리지 않는다.** Tab은 다음 칸으로 포커스를 옮기면서 앞 칸에 `change`를 낸다. 거기서 표를 다시 그리면 **방금 포커스가 들어간 칸이 지워져** 포커스가 엉뚱한 데로 튄다 — 칸 순서가 뒤죽박죽으로 느껴지는 원인이 이것이다.
     - 줄 안에서 칸이 생기거나 없어져야 하면(조 이름에 따라 재료원가 칸이 옮겨가는 것처럼) **그 칸만 바꿔 끼운다**(`syncCostCells`). 칸이 미리 있어야 Tab이 조 → 항목 → 단가 → 재료원가로 이어진다. 바꿔 끼울 때 **옛 칸을 먼저 지우고** 새 칸을 넣는다 — 순서를 바꾸면 방금 넣은 칸이 지워진다.
