...
- 계산은 `compute()` 한 곳에 모아 두고 화면·CSV·시트 전송이 모두 그 결과를 쓴다. 화면 그리기와 섞지 말 것.
  - 조별 교환권 매출 = `finals` − `floats`, 여기에 `cash`를 더해야 그 조의 실제 매출
  ...
  - 매출 = 받은 돈 + 조별 현금 매출 합계 + 행사전 구매 매출. **조가 모아 온 교환권은 더하지 않는다** — 이미 받은 돈에 들어 있어 두 번 세는 것이 된다
...
- **구글 시트 연동**: [저장·백업] 탭에서 `assets/config.js`의 Apps Script 주소로 `saveSettlement`/`loadSettlement`를 보낸다. 시트의 「정산」 탭은 사람이 읽는 표, 「정산_원본」 탭은 다시 불러오기 위한 JSON 한 칸이며 **보낼 때마다 통째로 덮어쓴다**. 암호는 `assets/answers.gs` 쪽에만 있다.
