...
- 상태는 `app.js` 최상단의 단일 `state` 객체(`booths`, `floats`, `finals`, `cash`, `costs`, `commonCosts`, `presale`, `desk`)로 관리하며, `localStorage` 키 `cbz_voucher_v1`에 저장된다. 권종은 `DENOMS = [1000, 5000, 10000]`으로 고정.
...
- 계산은 `compute()` 한 곳에 모아 두고 화면·CSV·시트 전송이 모두 그 결과를 쓴다. 화면 그리기와 섞지 말 것.
  ...
  - 팔려 나간 교환권 = (아침 통 − 저녁 통) − 조별 지급 합계
  - 받은 돈 = (저녁 금고 − 아침 금고) + 계좌이체 → **둘의 차이가 0이어야 한다**
